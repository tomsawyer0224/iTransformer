...
class TimeSeriesDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        y = {
            k: self.df[idx + self.lookback_len : idx + self.lookback_len + k]
            for k in self.pred_length
        }
        ...
        """
        x = torch.tensor(x).float()  # float32
        y = {k: torch.tensor(v).float() for k, v in y.items()}  # float32
        return x, y
...
class TimeSeriesDataModule(L.LightningDataModule):
    ...
    def collate_fn(examples):
        ...
        x, y = zip(*examples)
        x = torch.stack(x)
        y = {k: torch.stack([yi[k] for yi in y]) for k in y[0].keys()}
        return x, y
