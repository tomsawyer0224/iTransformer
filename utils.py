    ...
    for k in pred_lengths:
        for i in range(num_variates):
            ...
            save_to = os.path.join(
                save_dir, prefix + "_" + time_series_names[i] + f"_{k}.png"
            )
...
def plot_results(
    ...
    for i in range(batch_size):
        ...
        single_next_ground_truth = {k: v[i] for k, v in next_ground_truths.items()}
        single_next_prediction = {k: v[i] for k, v in next_predictions.items()}
        plot_single_result(
